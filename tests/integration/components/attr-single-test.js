...
test('render usual attribute', function(assert) {
  ...
  assert.equal(this.$('tr td:nth-child(1)').text(), 'foo');
  assert.equal(this.$('tr td:nth-child(2)').text(), 'bar');
  assert.equal(this.$('tr td:nth-child(3)').text().trim(), 'DELETE');
});
...
test('render attribute after marking as deleted', function(assert) {
  ...
  assert.equal(this.$('tr td:nth-child(3)').text().trim(), '[DELETING]');
});
...
test('render new attribute which is not in backend yet', function(assert) {
  ...
  assert.equal(this.$('tr td:nth-child(3)').text().trim(), '[ADDING]');
});
