...
test('it renders nothing when there are no data', function(assert) {
  ...
  assert.equal(this.$().text().trim(), '', 'Data are not entered');

  this.set('empty', []);
  this.render(hbs`{{filter-list data=empty}}`);
  assert.equal(this.$().text().trim(), '', 'Data do not contain any record');
  ...
  this.render(hbs`
    ...
    {{/filter-list}}
  `);
  assert.equal(this.$().text().trim(), '', 'Inserted content was ignored');
});

test('it renders single item', function(assert) {
  this.set('list', [{title: 'first'}]);
  this.render(hbs`{{filter-list data=list}}`);
  assert.equal(this.$().text().trim(), 'first', 'Single item was rendered');
});
