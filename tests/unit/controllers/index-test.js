...
skip('check action onClick with component ID', function (assert) {
  ...
  assert.equal(
    undefined,
    controller.get('selectedComponentId'),
    'Initially, there is not a selected component',
  );
  controller.actions.onClick.call(controller, undefined, 'menu:foo');
  assert.equal(
    'menu:foo',
    controller.get('selectedComponentId'),
    'Select a component that is not backed by real component',
  );
  controller.actions.onClick.call(controller, 'abc', 'menu:foo');
  assert.equal(
    'menu:foo',
    controller.get('selectedComponentId'),
    'Select a component that is backed by real component',
  );
  assert.equal(
    'abc',
    controller.get('selectedComponent'),
    'Select a component that is backed by real component',
  );
});
